-- R44R37R2R1 — Production geography schema compatibility recovery
-- Root cause: some production databases have the legacy installation_neighborhoods schema without name_en.
-- This migration is additive/non-destructive and uses the already-approved National Address Jeddah/Makkah mapping.
-- It supersedes the failed R44R37R2 database step; do NOT rerun the failed R44R37R2 SQL after this migration succeeds.

begin;

-- Schema-compatibility bridge. These columns are display/master-data metadata only.
...
-- Authoritative region labels from the same National Address snapshot integrated in PETATOE.
...
where btrim(r.name)=v.name_ar
  and nullif(btrim(r.name_en),'') is null;

-- Current PETATOE operating cities covered by the approved district snapshot.
...
where btrim(c.name)=v.name_ar
  and nullif(btrim(c.name_en),'') is null;

create temporary table tmp_r44r37r2r1_authoritative_districts(
  city_source_id bigint not null,
  district_source_id bigint not null,
  name_ar text not null,
  name_en text not null
) on commit drop;

insert into tmp_r44r37r2r1_authoritative_districts(city_source_id,district_source_id,name_ar,name_en) values
...
-- Backfill the new metadata column without literal translation/transliteration.
-- First resolve every district name whose approved English label is unambiguous across the snapshot.
with normalized as (
  select a.*, regexp_replace(translate(btrim(a.name_ar),'أإآىة','ااايه'),'\s+','','g') as norm_ar
  from tmp_r44r37r2r1_authoritative_districts a
), canonical as (
  select norm_ar,min(name_en) as name_en
  from normalized
  group by norm_ar
  having count(distinct name_en)=1
), matched as (
  select n.id,c.name_en
  from public.installation_neighborhoods n
  join canonical c
    on regexp_replace(translate(btrim(n.name),'أإآىة','ااايه'),'\s+','','g')=c.norm_ar
...
-- Only three Arabic district names are ambiguous between Jeddah/Makkah in this approved snapshot.
-- If the legacy production table has the city text column, disambiguate those three with the stored city.
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='installation_neighborhoods' and column_name='city'
  ) then
    execute $sql$
      update public.installation_neighborhoods n
      set name_en=case
        when btrim(n.name)='حي العدل' and coalesce(n.city,'') like '%جدة%' then 'Al Adel Dist.'
        when btrim(n.name)='حي العدل' and coalesce(n.city,'') like '%مكة%' then 'Al Adl Dist.'
        when btrim(n.name)='حي السليمانية' and coalesce(n.city,'') like '%جدة%' then 'As Sulaymaniyah Dist.'
        when btrim(n.name)='حي السليمانية' and coalesce(n.city,'') like '%مكة%' then 'As Sulimaniyah Dist.'
        when btrim(n.name)='حي الشرائع' and coalesce(n.city,'') like '%مكة%' then 'Ash Sharia Dist.'
        else n.name_en
      end
      where nullif(btrim(n.name_en),'') is null
        and btrim(n.name) in ('حي العدل','حي السليمانية','حي الشرائع')
    $sql$;
  end if;
end $$;

-- Functional UI keys from the failed R44R37R2 transaction + R2R1 release metadata.
...
values
  ...
  ('pwa.update.release.r44r37r2r1.title','aboutApp','system','title','إصلاح توافق مخطط الأحياء في الإنتاج — R44R37R2R1','Production neighborhood schema compatibility recovery — R44R37R2R1','إصلاح توافق مخطط الأحياء في الإنتاج — R44R37R2R1','Production neighborhood schema compatibility recovery — R44R37R2R1',true,now()),
  ('pwa.update.release.r44r37r2r1.note1','aboutApp','system','note','إضافة حقل الاسم الإنجليزي الجغرافي بشكل آمن عند غيابه في قواعد الإنتاج القديمة دون حذف أو تغيير بيانات الأعمال.','Safely add missing geographic English-name metadata on legacy production schemas without deleting or changing business data.','إضافة حقل الاسم الإنجليزي الجغرافي بشكل آمن عند غيابه في قواعد الإنتاج القديمة دون حذف أو تغيير بيانات الأعمال.','Safely add missing geographic English-name metadata on legacy production schemas without deleting or changing business data.',true,now()),
  ('pwa.update.release.r44r37r2r1.note2','aboutApp','system','note','ملء أسماء أحياء جدة ومكة الإنجليزية من خريطة العنوان الوطني المعتمدة داخل بيتاتو بدل الترجمة الحرفية.','Backfill Jeddah and Makkah district English names from PETATOE’s approved National Address mapping instead of literal translation.','ملء أسماء أحياء جدة ومكة الإنجليزية من خريطة العنوان الوطني المعتمدة داخل بيتاتو بدل الترجمة الحرفية.','Backfill Jeddah and Makkah district English names from PETATOE’s approved National Address mapping instead of literal translation.',true,now()),
  ('pwa.update.release.r44r37r2r1.note3','aboutApp','system','note','استعادة مفاتيح واجهة R44R37R2 التي تراجعت تلقائيًا مع فشل المعاملة الأولى، مع الحفاظ على الخدمات والصلاحيات والمزامنة دون تعديل.','Restore the R44R37R2 UI keys rolled back with the failed transaction while keeping services, permissions, and sync logic unchanged.','استعادة مفاتيح واجهة R44R37R2 التي تراجعت تلقائيًا مع فشل المعاملة الأولى، مع الحفاظ على الخدمات والصلاحيات والمزامنة دون تعديل.','Restore the R44R37R2 UI keys rolled back with the failed transaction while keeping services, permissions, and sync logic unchanged.',true,now())
), upserted as (
...
-- Current UUID-safe neighborhood translation coverage, now reading from the compatibility column populated above.
insert into public.app_translations(
  translation_key,screen_key,module_name,text_type,ar_text,en_text,default_ar,default_en,is_active,updated_at
)
select
