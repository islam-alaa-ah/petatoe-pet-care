-- PETATOE P5.13.8.72 R44R38R17R1 — SEA VIBE Asset Expense VAT & Payment Source Recovery
-- Scope:
...
-- Safety:
--   Recovery for R44R38R17 failure caused by the closed-trip expense mutation guard during snapshot backfill.
--   The guard is relaxed only for the exact amount_inclusive initialization, then restored in the same transaction.
...
-- R17R1 recovery: closed trip expenses are immutable to users, but this migration
-- needs to initialize a newly introduced financial snapshot without changing any
-- historical business value. Temporarily permit ONLY that exact initialization.
create or replace function public.sea_vibe_guard_trip_expense_mutation() returns trigger
language plpgsql security definer set search_path=public as $$
declare v_trip_id uuid; v_status text; v_system boolean; v_snapshot_init boolean:=false;
begin
  v_trip_id:=coalesce(new.trip_id,old.trip_id);
  v_system:=coalesce(new.is_system_generated,old.is_system_generated,false);
  if tg_op='UPDATE' then
    v_snapshot_init:=old.amount_inclusive is null
      and new.amount_inclusive is not distinct from old.amount
      and new.tax_code is not distinct from old.tax_code
      and new.tax_amount is not distinct from old.tax_amount
      and new.payment_account_id is not distinct from old.payment_account_id
      and new.expense_scope is not distinct from old.expense_scope
      and new.trip_id is not distinct from old.trip_id
      and new.asset_id is not distinct from old.asset_id
      and new.expense_catalog_id is not distinct from old.expense_catalog_id
      ...
      and new.is_system_generated is not distinct from old.is_system_generated;
  end if;
  if v_trip_id is not null and not v_system then
    select status into v_status from public.sea_vibe_trips where id=v_trip_id;
    if v_status='closed' and not v_snapshot_init then raise exception 'trip_closed'; end if;
  end if;
...
-- Restore the canonical closed-trip guard immediately after the one-time snapshot backfill.
create or replace function public.sea_vibe_guard_trip_expense_mutation() returns trigger
language plpgsql security definer set search_path=public as $$
...
-- ---------------------------------------------------------------------------
select
  'R44R38R17R1_ASSET_EXPENSE_VAT_PAYMENT_SOURCE_RECOVERY_OK'::text as status,
  (select case when pg_get_functiondef(p.oid) ilike '%if v_status=''closed'' then raise exception ''trip_closed''%'
                 and pg_get_functiondef(p.oid) not ilike '%v_snapshot_init%'
          then 1 else 0 end::integer
   from pg_proc p join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public' and p.proname='sea_vibe_guard_trip_expense_mutation' limit 1) as closed_trip_guard_restored,
