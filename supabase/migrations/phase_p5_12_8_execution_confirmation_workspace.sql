-- P5.12.8 — Execution Confirmation Workspace
-- Allows completion-authorized users to update service lines and collection data without replacing confirmed visit history.
begin;
...
declare
  ...
  v_reference text:=nullif(btrim(coalesce(p_collection->>'collection_reference','')),'');
...
begin
  ...
  insert into public.installation_request_collection(installation_request_id,session_value,total_discount,amount_collected,collection_status,payment_method,appointment_status,collection_reference,collection_notes,updated_at)
  values(p_request_id,v_final,v_discount,v_amount,case when v_amount<=0 then 'غير محصل' when v_amount>=v_final-0.005 then 'محصل بالكامل' else 'محصل جزئيًا' end,v_payment,coalesce(r.status,'قيد التنفيذ'),v_reference,v_notes,now())
  on conflict(installation_request_id) do update set session_value=excluded.session_value,total_discount=excluded.total_discount,amount_collected=excluded.amount_collected,collection_status=excluded.collection_status,payment_method=excluded.payment_method,appointment_status=excluded.appointment_status,collection_reference=excluded.collection_reference,collection_notes=excluded.collection_notes,updated_at=now();
