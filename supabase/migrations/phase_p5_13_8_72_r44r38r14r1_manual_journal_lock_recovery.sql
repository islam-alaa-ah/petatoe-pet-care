-- PETATOE P5.13.8.72 R44R38R14R1 — SEA VIBE Manual Journal Lock Recovery
-- Recovery for R44R38R14 production deadlock while creating a SELECT policy on sea_vibe_chart_accounts.
-- Historical R44R38R14 remains immutable. This migration is intentionally re-runnable.
-- It avoids adding a journal policy to the shared chart table; journal account lookup is exposed only through
-- a permission-checked SECURITY DEFINER RPC. DDL is split into short transactions to reduce production lock duration.

-- Preflight: never silently widen chart-account table access if a prior deployment created the old policy.
do $$
begin
  if exists (
    ...
    raise exception 'R44R38R14R1_LEGACY_CHART_POLICY_PRESENT_REQUIRES_REVIEW';
  end if;
end;
$$;
...
begin
  ...
  if v_vat<>1 or v_tables<>3 or v_screen<>1 or v_rpc<>1 or v_unsafe_chart_policy<>0
    ...
    raise exception 'R44R38R14R1_VERIFICATION_FAILED vat=% tables=% screen=% rpc=% unsafe_chart_policy=% journal_policies=% storage_policies=% bucket=%',
...
select
  'R44R38R14R1_JOURNAL_LOCK_RECOVERY_OK'::text as status,
