-- PETATOE P5.13.8.72 R44R38R13R1 — Treasury View Numeric Compatibility Recovery
-- Recovery for R44R38R13 after PostgreSQL rejected CREATE OR REPLACE VIEW because the UNION output
-- inferred voucher amount as unconstrained numeric instead of the existing numeric(14,2) view column.
-- Re-applies the R44R38R13 objects idempotently and preserves the historical R44R38R13 migration unchanged.
-- No DELETE/TRUNCATE/DROP, no permission widening, no Offline Queue / Sync Engine changes, no pruning changes.
...
from (values
  ...
  ('pwa.update.release.r44r38r13r1.title','systemSettings','pwa','title','استعادة توافق عرض خزينة SEA VIBE — R44R38R13R1','Recover SEA VIBE Treasury View Compatibility — R44R38R13R1'),
  ('pwa.update.release.r44r38r13r1.note1','systemSettings','pwa','note','إصلاح توافق نوع عمود amount في عرض حركات خزينة SEA VIBE بدون تعديل Migration R13 التاريخية.','Fixes the amount-column type compatibility in the SEA VIBE treasury movements view without modifying the historical R13 migration.'),
  ('pwa.update.release.r44r38r13r1.note2','systemSettings','pwa','note','إعادة تطبيق مكونات سندات القبض والصرف بشكل آمن بعد فشل R13 داخل Transaction.','Safely reapplies receipt/payment voucher objects after the R13 transaction failed.'),
  ('pwa.update.release.r44r38r13r1.note3','systemSettings','pwa','note','لا تغيير في الحسابات أو Offline/Sync أو الصلاحيات أو R44 Pruning.','No changes to calculations, Offline/Sync, permissions, or R44 Pruning.')
...
select
  'R44R38R13R1_TREASURY_VIEW_NUMERIC_RECOVERY_OK'::text as status,
