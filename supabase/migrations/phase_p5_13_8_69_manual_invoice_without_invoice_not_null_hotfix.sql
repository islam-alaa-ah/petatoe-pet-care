-- Phase P5.13.8.69 — Manual invoice without-invoice NOT NULL hotfix
-- Root cause: sales_invoices.invoice_number is intentionally NOT NULL, while the P5.13.8.56
-- manual-invoice RPC inserted NULL when is_without_invoice=true. Other canonical invoice paths
-- already use an internal NOINV-* token while exposing the semantic state through is_without_invoice.
-- Scope: replace only create_manual_sales_invoice; no schema relaxation and no fake user-visible number.
begin;
...
begin
  ...
  insert into public.sales_invoices(
    ...
    v_invoice_number,null,p_customer_id,c.representative_id,
    null,null,null,
