-- PETATOE P5.13.8.72 R44R38R14R5 — SEA VIBE Journal Inclusive VAT
-- Scope:
--   1) Treat VAT (15%) manual journal amounts as VAT-inclusive.
--   2) Authoritatively split the inclusive amount into net + VAT inside the database RPC.
--   3) Keep generated VAT on the same debit/credit side as the source line.
-- Safety:
--   No schema changes, no RLS/permission widening, no destructive DDL,
--   no historical migration edits, no account 2101 relocation, no pruning changes.
...
declare
  ...
  v_net_debit numeric(14,2);
  v_net_credit numeric(14,2);
  v_inclusive_amount numeric(14,2);
...
begin
  ...
  -- Validate manual lines and calculate authoritative totals.
  -- VAT inputs are inclusive: net = inclusive / 1.15; VAT = inclusive - net.
  ...
  loop
    ...
    if v_tax_code='vat15' and (v_debit>0 or v_credit>0) then
      ...
      if v_account_id<>v_vat_account_id then
        v_inclusive_amount:=greatest(v_debit,v_credit);
        v_net_amount:=round(v_inclusive_amount/1.15,2);
        v_tax_amount:=round(v_inclusive_amount-v_net_amount,2);
        if v_debit>0 then v_net_debit:=v_net_amount; else v_net_credit:=v_net_amount; end if;
      end if;
    end if;

    v_valid_lines:=v_valid_lines+1;
    v_total_debit:=v_total_debit+v_net_debit;
    v_total_credit:=v_total_credit+v_net_credit;
  ...
  loop
    ...
    insert into public.sea_vibe_journal_entry_lines(
      ...
      v_tax_code,v_net_debit,v_net_credit,true,false,null
    )
...
-- Preserve the exact R14/R14R4 execution boundary; no widening.
...
select
  'R44R38R14R5_JOURNAL_INCLUSIVE_VAT_OK'::text as status,
  (select count(*)::integer
   ...
   where a.account_code='2101'
     ...
     and a.root_class=2
     and a.allow_posting=true
     and a.is_active=true) as vat_account_linked,
  (select case
     when pg_get_functiondef(p.oid) ilike '%v_net_amount:=round(v_inclusive_amount/1.15,2)%'
      and pg_get_functiondef(p.oid) ilike '%v_tax_amount:=round(v_inclusive_amount-v_net_amount,2)%'
      and pg_get_functiondef(p.oid) not ilike '%greatest(v_debit,v_credit)*0.15%'
     then 1 else 0 end::integer
   from pg_proc p
   join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public'
     and p.proname='sea_vibe_save_manual_journal_r44r14'
   limit 1) as inclusive_vat_formula,
  (select case
     when pg_get_functiondef(p.oid) ilike '%v_tax_code,v_net_debit,v_net_credit,true,false,null%'
      and pg_get_functiondef(p.oid) ilike '%case when v_debit>0 then v_tax_amount else 0 end%'
      and pg_get_functiondef(p.oid) ilike '%case when v_credit>0 then v_tax_amount else 0 end%'
     then 1 else 0 end::integer
   from pg_proc p
   join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public'
     and p.proname='sea_vibe_save_manual_journal_r44r14'
   limit 1) as net_and_same_side_posting,
  (select count(*)::integer
   from pg_policies
   where schemaname='public'
     and tablename='sea_vibe_chart_accounts'
     and policyname='sea vibe chart accounts journal read') as unsafe_chart_policy;
