...
-- ============================================================
-- D0. Existing schedule RPC return-type compatibility recovery
-- ============================================================
-- PostgreSQL cannot CREATE OR REPLACE a function when the return type changed.
-- Production currently has an older no-argument function with a different
-- return contract. Preserve it under a legacy name instead of CASCADE dropping
-- it, then create the canonical PETATOE JSONB RPC under the expected name.
do $$
declare
  v_oid oid;
  v_result text;
...
begin
  ...
  if v_oid is not null and lower(coalesce(v_result,'')) <> 'jsonb' then
    v_legacy_name := 'get_installation_schedule_global_legacy_' ||
      to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS');

    execute format(
      'alter function public.get_installation_schedule_global() rename to %I',
      v_legacy_name
    );
  end if;
end $$;
