-- P5.13.8.30 — Collection canonical remaining-scope recovery
-- Deep audit recovery: UI and database now use the same collectable amount.
-- For an execution group, collectable = min(group financial due, request actual remaining).
-- This blocks true over-collection while allowing the exact amount shown to the user.
...
begin
  ...
  if p_visit_id is not null then
    ...
    remaining_amount:=least(group_due,request_remaining);
