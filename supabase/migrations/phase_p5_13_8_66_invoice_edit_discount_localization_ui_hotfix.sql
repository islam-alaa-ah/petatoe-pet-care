-- P5.13.8.66 — Invoice Edit Discount + Localization/UI Hotfix
-- Scope: preserve the P5.13.8.65 canonical full-edit path while adding editable discount intent
-- and the missing canonical localization row for the unit-price header.
begin;
...
begin
  ...
  if inv.source_type='manual' then
    select jsonb_build_object(
      ...
      'discount',jsonb_build_object('type',case when inv.discount_type='percentage' then 'percentage' else 'amount' end,'value',coalesce(inv.discount_value,inv.discount_amount,0),'amount',coalesce(inv.discount_amount,0)),
  ...
  select jsonb_build_object(
    ...
    'discount',jsonb_build_object('type',case when req.discount_type='percentage' then 'percentage' else 'amount' end,'value',coalesce(req.discount_value,req.discount_amount,0),'amount',coalesce(req.discount_amount,0)),
...
drop function if exists public.update_sales_invoice_full_v1(uuid,text,date,boolean,text,jsonb,text,uuid[]);
...
begin
  ...
  v_discount_type:=case when p_discount_type='percentage' then 'percentage' else 'amount' end;
  v_discount_value:=greatest(coalesce(p_discount_value,0),0);
  if v_discount_type='percentage' then v_discount_value:=least(v_discount_value,100); end if;
  ...
  if inv.source_type='manual' then
    ...
    if v_discount_type='percentage' then
      v_discount_amount:=round((subtotal+tax)*v_discount_value/100.0,2);
    else
      v_discount_amount:=least(v_discount_value,subtotal+tax);
    end if;
    final_total:=round(greatest((subtotal+tax)-v_discount_amount,0),2);
    update public.sales_invoices set invoice_number=v_invoice_number,is_without_invoice=coalesce(p_without_invoice,false),invoice_date=p_invoice_date,payment_method=v_payment,
      manual_subtotal=subtotal,tax_rate=15,tax_amount=tax,discount_type=v_discount_type,discount_value=v_discount_value,discount_amount=v_discount_amount,
      final_amount=final_total,invoice_amount=round(final_total/1.15,2),notes=nullif(btrim(coalesce(p_collection_notes,'')),''),updated_at=now() where id=inv.id;
    ...
    -- Discount intent belongs to the canonical installation request. Its existing financial trigger
    -- recalculates VAT, discount_amount, and final_amount using the same appointment formula.
    update public.installation_requests
    set discount_type=v_discount_type,discount_value=v_discount_value,updated_at=now()
    where id=inv.installation_request_id;
...
revoke all on function public.update_sales_invoice_full_v1(uuid,text,date,boolean,text,jsonb,text,numeric,text,uuid[]) from public,anon;
grant execute on function public.update_sales_invoice_full_v1(uuid,text,date,boolean,text,jsonb,text,numeric,text,uuid[]) to authenticated,service_role;
...
values ('appointmentNew.services.unitPrice','installationRequestNew','appointments','label','سعر الوحدة','Unit Price','سعر الوحدة','Unit Price',true)
on conflict (translation_key) do update set
  ar_text=excluded.ar_text,en_text=excluded.en_text,default_ar=excluded.default_ar,default_en=excluded.default_en,is_active=true;
