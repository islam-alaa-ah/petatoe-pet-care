...
-- P5.13.8.37
-- Keep completion-workspace money validation at canonical 2-decimal SAR precision.
-- This prevents a displayed valid total from being rejected because a raw numeric
-- path and the UI did not compare the exact same halala value.
...
begin
  ...
  -- Canonical money precision: the UI and DB must validate the same 2-decimal SAR value.
  v_amount:=round(coalesce(v_amount,0),2);
  v_final:=round(coalesce(v_final,0),2);
  v_discount:=round(coalesce(v_discount,0),2);
  if (v_amount*100)::bigint>(v_final*100)::bigint then raise exception 'Collected amount cannot exceed appointment total' using errcode='23514'; end if;
  ...
  values(p_request_id,v_final,v_discount,v_amount,case when (v_amount*100)::bigint<=0 then 'غير محصل' when (v_amount*100)::bigint>=(v_final*100)::bigint then 'محصل بالكامل' else 'محصل جزئيًا' end,v_payment,coalesce(r.status,'قيد التنفيذ'),v_invoice_number,v_notes,now())
