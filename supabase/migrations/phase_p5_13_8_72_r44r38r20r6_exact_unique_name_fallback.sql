-- PETATOE R44R38R20R6
-- Exact Unique Name Fallback + Identifier Conflict Guards
--
-- Approved matching hierarchy:
--   1) Exact / normalized legacy customer code
--   2) Exact normalized mobile
--   3) Exact UNIQUE normalized customer name (guarded fallback only)
--
-- Critical safety rules:
--   * Name never overrides an explicit-but-unmatched customer code.
--   * A mobile that belongs to Customer A cannot be accepted when the exact
--     unique source name belongs to Customer B; that becomes Needs Review.
--   * Code/name conflicts also become Needs Review.
--   * Name fallback requires exactly ONE current customer with that exact
--     normalized name.
--   * #ERROR! source identities are never name-matched automatically.
--
-- Scope: one canonical matcher only.
-- No customer writes. No invoice writes. No RLS / permission changes.
...
begin
  if (select count(*) from public.appointment_historical_sales_import_batches) <> 0
    ...
    raise exception 'R44R38R20R6_SAFETY_STOP: historical import data already exists';
  end if;

  if to_regprocedure('public.appointment_historical_customer_match_r44r38r20(text)') is null then
    raise exception 'R44R38R20R6_SAFETY_STOP: canonical customer matcher is missing';
  end if;
end
$$;
...
-- Preserve existing execution boundary.
...
select
  case
    ...
    then 'R44R38R20R6_EXACT_UNIQUE_NAME_FALLBACK_OK'
    else 'R44R38R20R6_EXACT_UNIQUE_NAME_FALLBACK_REVIEW'
  end as status,
