...
-- P5.13.8.71 R3 — Completion financial/collection synchronization
-- Canonical owner: public.update_installation_completion_workspace.
-- Keeps request-service identity logic from P5.13.8.64 unchanged while aligning completion edits with
-- the appointment financial trigger: VAT is calculated on subtotal first, then discount is deducted
-- from the VAT-inclusive gross. Completion discount edits are persisted as fixed-amount intent.
-- A previously fully-collected appointment may auto-clamp collection downward when an edit reduces
-- the final total; partial/manual collection edits keep the existing validation guard.
...
declare
  ...
  v_final numeric;
  v_discount numeric;
  v_previous_final numeric:=0;
  v_previous_amount numeric:=0;
  v_previous_status text:='';
  v_was_fully_collected boolean:=false;
begin
  ...
  v_previous_final:=round(greatest(coalesce(r.final_amount,0),0),2);
  select round(greatest(coalesce(c.amount_collected,0),0),2),coalesce(c.collection_status,'')
  into v_previous_amount,v_previous_status
  ...
  v_was_fully_collected:=v_previous_status='محصل بالكامل'
    or (v_previous_final>0 and (v_previous_amount*100)::bigint>=(v_previous_final*100)::bigint);
  ...
  set discount_type='amount',
      discount_value=greatest(coalesce(p_discount_amount,0),0),
  ...
  if v_was_fully_collected
     and (v_amount*100)::bigint=(v_previous_amount*100)::bigint
     and (v_amount*100)::bigint>(v_final*100)::bigint then
    v_amount:=v_final;
  end if;
...
insert into public.app_translations(
  ...
  'appointments.completion.validation.collectionExceedsFinal','installationCompletion','appointments','error',
  'تعذر الحفظ: المبلغ المحصل أكبر من الإجمالي النهائي بعد تعديل الخدمات أو الأسعار أو الخصم. راجع قيمة التحصيل قبل الحفظ.',
  'Unable to save: the collected amount exceeds the final appointment total after the service, price, or discount changes. Review the collected amount before saving.',
  'تعذر الحفظ: المبلغ المحصل أكبر من الإجمالي النهائي بعد تعديل الخدمات أو الأسعار أو الخصم. راجع قيمة التحصيل قبل الحفظ.',
  'Unable to save: the collected amount exceeds the final appointment total after the service, price, or discount changes. Review the collected amount before saving.',
  true,now()
