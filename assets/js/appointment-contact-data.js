(function(){
  ...
  async function load(){
    ...
    if($('appointmentContactWorkDate'))$('appointmentContactWorkDate').textContent=formatDate(state.workDate);
    configureAction();
    try{
  ...
  async function save(event){
    ...
    try{
      ...
      state.busy=true;configureAction();showStatus(t('appointments.contact.saving',"Saving today's data..."),'info');
      const row=await window.AppointmentContactDataService.saveForDate(state.workDate||todayIso(),payload,state.existing);
      state.existing=true;state.snapshot=row;setValues(row);showStatus(t('appointments.contact.saved',"Today's data was saved successfully."),'success');
  ...
  function onLanguage(){if($('appointmentContactWorkDate'))$('appointmentContactWorkDate').textContent=formatDate(state.workDate||todayIso());if($('appointmentContactUserName'))$('appointmentContactUserName').textContent=currentUserName();renderSummary();}
  function bind(){
    ...
    if(window.KYUMNavigation?.current?.()===VIEW)load();
  }
