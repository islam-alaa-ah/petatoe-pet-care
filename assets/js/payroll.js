(function(){
  ...
  const latinDigits=value=>window.PetatoeLocalization?.latinDigits?.(value)??String(value??'').replace(/[٠-٩]/g,d=>String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))).replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d)));
  function normalizeTierNumberInput(value){
    const raw=latinDigits(value).replace(/٫/g,'.').replace(/[٬,]/g,'').replace(/[^0-9.]/g,'');
    const first=raw.indexOf('.');
    return first<0?raw:`${raw.slice(0,first+1)}${raw.slice(first+1).replace(/\./g,'')}`;
  }
  function tierNumberInput(name,value,{min='0',step='0.01',placeholder=''}={}){
    const english=lang()==='en',normalized=normalizeTierNumberInput(value);
    return `<input name="${esc(name)}" type="${english?'text':'number'}" ${english?'inputmode="decimal" data-payroll-tier-latin-number="1"':''} min="${esc(min)}" step="${esc(step)}" value="${esc(normalized)}"${placeholder?` placeholder="${esc(placeholder)}"`:''}>`;
  }
  ...
  function payrollCacheStatusMessage(kind){
    const status=svc()?.getReadStatus?.(kind);if(status?.source!=='cache')return '';
    const updatedAt=Number(status?.metadata?.updatedAt||status?.updatedAt||0);
    ...
    if(hours<24)return t('shared.cache.hours',`يتم عرض بيانات محفوظة محليًا — آخر مزامنة منذ ${hours} ساعة.`,{count:hours});
    const days=Math.floor(hours/24);
    return t('shared.cache.days',`يتم عرض بيانات محفوظة محليًا — آخر مزامنة منذ ${days} يوم.`,{count:days});
  }
  function applyPayrollReadStatus(id,kind){setStatus(id,payrollCacheStatusMessage(kind),'info');}
  ...
  async function loadManagement(forceStatus=true,resetPeriod=false){
    ...
    try{
      ...
      applyPayrollReadStatus('payrollManagementStatus','management');payrollCommissionPeriod(resetPeriod);renderManagement();
    }catch(error){setStatus('payrollManagementStatus',uiMessage(error?.message),'error');renderManagementEmpty();}
  }
  ...
  async function handleSalaryAction(id,action){
    ...
    catch(error){setStatus('payrollManagementStatus',uiMessage(error?.message),'error');}
  }

  // -----------------------------------------------------------------------
  // Salary self-service
  // -----------------------------------------------------------------------
  async function loadSalaryStatement(){setStatus('salaryStatementStatus',t('payroll.loading','جاري التحميل...'));try{state.salary=await svc().loadSalaryStatement();applyPayrollReadStatus('salaryStatementStatus','salary');renderSalaryStatement();}catch(error){setStatus('salaryStatementStatus',uiMessage(error?.message),'error');}}
  ...
  // -----------------------------------------------------------------------
  async function loadCommissions(options={}){const range=commissionRange(Boolean(options?.resetRange));if(!validCommissionRange(range))return;setStatus('commissionManagementStatus',t('commission.loading','جاري تحميل العمولات...'));try{state.commissions=await svc().loadCommissionsRange(range.from,range.to);applyPayrollReadStatus('commissionManagementStatus','commissions');renderCommissions();}catch(error){setStatus('commissionManagementStatus',uiMessage(error?.message),'error');$('commissionManagementBody').innerHTML=`<tr><td colspan="9" class="payroll-empty">${esc(t('payroll.empty','لا توجد بيانات.'))}</td></tr>`;}}
  ...
  const commissionVehicleKey=row=>String(row?.carId||row?.teamId||'');
  function filteredCommissionRows(){const rows=[...(state.commissions?.rows||[])];const search=String($('commissionSearch')?.value||'').trim().toLowerCase();const car=$('commissionCarFilter')?.value||'';const role=$('commissionRoleFilter')?.value||'';const eligibility=$('commissionEligibilityFilter')?.value||'';return rows.filter(r=>(!search||`${r.employeeName||''} ${r.carName||''} ${r.plateNumber||''}`.toLowerCase().includes(search))&&(!car||commissionVehicleKey(r)===car)&&(!role||r.role===role)&&(!eligibility||commissionEligibility(r)===eligibility));}
  ...
  // -----------------------------------------------------------------------
  async function loadCommissionStatement(){setStatus('commissionStatementStatus',t('commission.loading','جاري تحميل العمولات...'));try{state.commissionStatement=await svc().loadCommissionStatement();applyPayrollReadStatus('commissionStatementStatus','commissionStatement');renderCommissionStatement();}catch(error){setStatus('commissionStatementStatus',uiMessage(error?.message),'error');}}
  ...
  // Reference data
  // -----------------------------------------------------------------------
  async function loadReference(){setStatus('payrollReferenceStatus',t('payroll.loading','جاري التحميل...'));try{state.reference=await svc().loadReference();applyPayrollReadStatus('payrollReferenceStatus','reference');renderReference();}catch(error){setStatus('payrollReferenceStatus',uiMessage(error?.message),'error');}}
  ...
  function tierCard(tier){return `<form class="tier-card-grid" data-tier-form="${esc(tier.id)}" data-role="${esc(tier.role)}" data-tier-no="${tier.tierNo}"><h5 class="full-span">${esc(tierLabel(tier.tierNo))}</h5><label><span>${esc(t('commission.tier.from','من'))}</span>${tierNumberInput('from',Number(tier.fromAmount||0),{min:'0',step:'0.01'})}</label><label><span>${esc(t('commission.tier.to','إلى'))}</span>${tierNumberInput('to',tier.toAmount==null?'':Number(tier.toAmount),{min:'0',step:'0.01',placeholder:'∞'})}</label><label><span>${esc(t('commission.tier.rate','النسبة %'))}</span>${tierNumberInput('rate',Number(tier.ratePercent||0),{min:'0',step:'0.0001'})}</label><label class="switch-row"><span>${esc(t('commission.tier.active','تفعيل الشريحة'))}</span><span class="payroll-switch"><input name="active" type="checkbox" ${tier.isActive?'checked':''}><span>${tier.isActive?esc(t('payroll.common.active','مفعلة')):esc(t('payroll.common.inactive','غير مفعلة'))}</span></span></label><button class="primary-btn full-span" type="submit" data-permission-screen="payrollReference" data-permission-action="edit">${esc(t('common.save','حفظ'))}</button></form>`;}
  ...
  function bind(){
    ...
    $('payrollReferenceRefresh')?.addEventListener('click',loadReference);$('payrollReferenceType')?.addEventListener('change',renderReference);$('payrollEmployeeAdd')?.addEventListener('click',()=>openEmployeeDialog());$('payrollReferenceContent')?.addEventListener('click',e=>{const btn=e.target.closest('[data-payroll-employee-edit]');if(!btn)return;openEmployeeDialog((state.reference?.employees||[]).find(x=>x.id===btn.dataset.payrollEmployeeEdit));});$('payrollReferenceContent')?.addEventListener('input',e=>{const input=e.target.closest('[data-payroll-tier-latin-number]');if(!input||lang()!=='en')return;const normalized=normalizeTierNumberInput(input.value);if(normalized!==input.value)input.value=normalized;});$('payrollReferenceContent')?.addEventListener('submit',async e=>{const form=e.target.closest('[data-tier-form]');if(!form)return;e.preventDefault();const fromRaw=normalizeTierNumberInput(form.elements.from.value),toRaw=normalizeTierNumberInput(form.elements.to.value),rateRaw=normalizeTierNumberInput(form.elements.rate.value);const record={role:form.dataset.role,tierNo:Number(form.dataset.tierNo),fromAmount:Number(fromRaw||0),toAmount:toRaw===''?null:Number(toRaw),ratePercent:Number(rateRaw||0),isActive:form.elements.active.checked};setStatus('payrollReferenceStatus',t('payroll.saving','جاري الحفظ...'));try{state.reference=await svc().saveTier(record);renderReference();setStatus('payrollReferenceStatus',t('commission.tier.saved','تم حفظ الشريحة.'),'success');}catch(error){setStatus('payrollReferenceStatus',uiMessage(error?.message),'error');}});
    ...
    window.addEventListener('payroll-data-updated',e=>{const kind=e.detail?.kind,current=svc()?.getCache?.()||{};if(kind==='management'&&!$('payrollManagementView')?.classList.contains('hidden')&&current.management){state.management=current.management;payrollCommissionPeriod(false);renderManagement();applyPayrollReadStatus('payrollManagementStatus','management');}if(kind==='salary'&&!$('salaryStatementView')?.classList.contains('hidden')&&current.salary){state.salary=current.salary;renderSalaryStatement();applyPayrollReadStatus('salaryStatementStatus','salary');}if(kind==='commissions'&&!$('commissionManagementView')?.classList.contains('hidden')&&current.commissions){state.commissions=current.commissions;renderCommissions();applyPayrollReadStatus('commissionManagementStatus','commissions');}if(kind==='commissionStatement'&&!$('commissionStatementView')?.classList.contains('hidden')&&current.commissionStatement){state.commissionStatement=current.commissionStatement;renderCommissionStatement();applyPayrollReadStatus('commissionStatementStatus','commissionStatement');}if(kind==='reference'&&!$('payrollReferenceView')?.classList.contains('hidden')&&current.reference){state.reference=current.reference;renderReference();applyPayrollReadStatus('payrollReferenceStatus','reference');}if(kind==='adjustmentCatalog'&&current.adjustmentCatalog){state.adjustmentCatalog=current.adjustmentCatalog;renderAdjustmentCatalog();}});
