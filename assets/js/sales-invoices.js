(()=>{
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    $("salesInvoiceEditForm")?.addEventListener("submit",async e=>{e.preventDefault();if(!editingInvoice)return;const btn=$("saveSalesInvoiceEdit");btn.disabled=true;try{const withoutInvoice=Boolean($("salesInvoiceEditNoInvoice").checked),invoiceNumber=$("salesInvoiceEditNumber").value.trim(),invoiceDate=$("salesInvoiceEditDate").value;if(!withoutInvoice&&!invoiceNumber)throw new Error(t("invoices.edit.numberValidation","رقم الفاتورة مطلوب أو اختر بدون فاتورة."));await window.SalesInvoicesService.updateInvoice({id:editingInvoice.id,invoiceNumber,invoiceDate,withoutInvoice});$("salesInvoiceEditDialog").close();await load()}catch(err){const el=$("salesInvoiceEditStatus");el.textContent=err.message;el.classList.remove("hidden");el.classList.add("error")}finally{btn.disabled=false}});
