(()=>{
  ...
  function openEdit(r){editingInvoice=r;$("salesInvoiceEditRequestLabel").textContent=`${r.requestNumber} — ${r.customerName}`;$("salesInvoiceEditNumber").value=r.isWithoutInvoice?"":r.storedInvoiceNumber;$("salesInvoiceEditDate").value=r.invoiceDate||"";$("salesInvoiceEditNoInvoice").checked=Boolean(r.isWithoutInvoice);syncEditNoInvoice();$("salesInvoiceEditStatus").textContent="";$("salesInvoiceEditStatus").classList.add("hidden");$("salesInvoiceEditDialog")?.showModal()}
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    $("salesInvoiceEditNumber")?.addEventListener("input",e=>{e.target.value=e.target.value.replace(/\D/g,"").slice(0,9)});
    ...
    $("salesInvoiceEditForm")?.addEventListener("submit",async e=>{e.preventDefault();if(!editingInvoice)return;const btn=$("saveSalesInvoiceEdit");btn.disabled=true;try{const withoutInvoice=Boolean($("salesInvoiceEditNoInvoice").checked),invoiceNumber=$("salesInvoiceEditNumber").value.trim(),invoiceDate=$("salesInvoiceEditDate").value;if(!withoutInvoice&&invoiceNumber.length!==9)throw new Error(t("invoices.edit.numberValidation","رقم الفاتورة يجب أن يكون 9 أرقام، أو اختر بدون فاتورة."));await window.SalesInvoicesService.updateInvoice({id:editingInvoice.id,invoiceNumber,invoiceDate,withoutInvoice});$("salesInvoiceEditDialog").close();await load()}catch(err){const el=$("salesInvoiceEditStatus");el.textContent=err.message;el.classList.remove("hidden");el.classList.add("error")}finally{btn.disabled=false}});
