(()=>{
  ...
  const normalize=r=>{const invoiceAmount=Number(r.invoice_amount||0);return {id:r.id,requestNumber:r.request_number||"—",customerName:r.customer?.customer_name||"—",invoiceNumber:r.invoice_number||"",invoiceAmount,invoiceAmountInclTax:Math.round(invoiceAmount*1.15*100)/100,installationExpenses:Number(r.installation_expenses||0),representativeName:r.representative?.full_name||"—",invoiceDate:r.invoice_date||"",sourceType:r.source_type||"quotation",status:r.status||"صادرة",quotationId:r.quotation_id||"",installationRequestId:r.installation_request_id||"",installationExecutionVisitId:r.installation_execution_visit_id||""}};
