(()=>{
  ...
  async function list(){
    ...
    return (data||[]).map(r=>{const all=byRequest.get(String(r.installation_request_id))||[];const attachments=r.installation_execution_visit_id?all.filter(f=>!f.visitId||String(f.visitId)===String(r.installation_execution_visit_id)):all;return normalize({...r,payment_method:paymentByRequest.get(String(r.installation_request_id))||"",attachments})})
  }
  ...
  async function updateInvoice(payload){
    ...
    if(!withoutInvoice&&!invoiceNumber)throw new Error("رقم الفاتورة مطلوب أو اختر بدون فاتورة.");
    if(!invoiceDate)throw new Error("تاريخ الفاتورة مطلوب.");
    if(payload?.sourceType==="installation"&&!paymentMethod)throw new Error("طريقة الدفع مطلوبة لفاتورة الموعد.");
