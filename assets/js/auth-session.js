(function () {
  ...
  function loading(on) { const button = el().submit; if (!button) return; button.disabled = on; button.textContent = on ? window.PetatoeLocalization.t("auth.login.signingIn") : window.PetatoeLocalization.t("auth.login.submit"); }
  ...
  async function loadProfileOnline(userId) {
    ...
    if (error) throw new Error(window.PetatoeLocalization.t("auth.error.profileLoad", { error: error.message }));
    if (!data?.is_active) throw new Error(window.PetatoeLocalization.t("auth.error.inactive"));
    return data;
  }
  ...
  function applyIdentity(session, profile, offline) {
    ...
    if (e.userName) e.userName.textContent = profile.full_name || session.user.email || window.PetatoeLocalization.t("auth.common.user");
  ...
  async function initialize() {
    try {
      ...
      if (!opened) showLogin(error instanceof Error ? error.message : window.PetatoeLocalization.t("auth.error.sessionVerify"));
  ...
  async function signIn(email, password) {
    ...
    try {
      if (navigator.onLine === false) throw new Error(window.PetatoeLocalization.t("auth.error.offlineNewLogin"));
      if (!window.customerSupabase) throw new Error(window.customerSupabaseStatus?.reason || window.PetatoeLocalization.t("auth.error.supabaseNotReady"));
      ...
      const text = error?.message === "Invalid login credentials" ? window.PetatoeLocalization.t("auth.error.invalidCredentials") : (error instanceof Error ? error.message : window.PetatoeLocalization.t("auth.error.loginFailed"));
      message(text);
  ...
  async function signOut() {
    ...
    try { if (navigator.onLine !== false) await window.customerSupabase?.auth.signOut(); }
      ...
      showLogin(); message(window.PetatoeLocalization.t("auth.logout.success"), "success");
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    window.customerSupabase?.auth.onAuthStateChange(async (event, session) => {
      ...
      if (["SIGNED_IN", "USER_UPDATED"].includes(event) && session) {
        ...
        catch (error) { await window.customerSupabase.auth.signOut(); showLogin(error instanceof Error ? error.message : window.PetatoeLocalization.t("auth.error.sessionActivate")); }
      }
    });
  });
