(function(){
  ...
  async function load(force=false){if(!window.PetatoeLocalization)return;setStatus('جاري تحميل مركز الترجمه...');try{rows=await window.PetatoeLocalization.loadRemote(force);render();syncLanguage();setStatus('')}catch(e){setStatus(e.message||'تعذر تحميل مركز الترجمه.','error')}}
  ...
  function setLanguage(lang){window.PetatoeLocalization?.setLanguage?.(lang);syncLanguage();setStatus(lang==='en'?'تم تحويل شاشة تنفيذ المواعيد إلى الإنجليزية.':'تم تحويل شاشة تنفيذ المواعيد إلى العربية.','success');setTimeout(()=>setStatus(''),1800)}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    $('translationCenterHeaderBtn')?.addEventListener('click',open);
  ...
  window.TranslationCenterUI=Object.freeze({load,open});
})();
