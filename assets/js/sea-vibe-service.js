(function(){
  ...
  const mapRef=r=>({id:r.id,nameAr:r.name_ar||'',nameEn:r.name_en||'',isActive:r.is_active!==false,systemKey:r.system_key||'',isSystem:!!r.is_system,accountId:r.account_id||'',allowGeneral:r.allow_general!==false,allowTrip:r.allow_trip!==false,allowAsset:r.allow_asset===true,assetTreatment:r.asset_treatment==='capitalized'?'capitalized':'operating',fuelCostAmount:num(r.fuel_cost_amount),requiresCustomer:r.requires_customer!==false,serialSeriesKey:r.serial_series_key||'standard',updatedAt:r.updated_at||r.created_at||''});
  ...
  const mapAsset=r=>({id:r.id,code:r.asset_code||'',name:r.asset_name||'',initialValue:num(r.initial_value),capitalizedExpenses:num(r.capitalized_expenses),operatingExpenses:num(r.operating_expenses),currentValue:num(r.current_value??r.initial_value),notes:r.notes||'',isActive:r.is_active!==false,createdAt:r.created_at||'',updatedAt:r.updated_at||''});
  const mapExpense=r=>({id:r.id,scope:r.expense_scope,tripId:r.trip_id||'',assetId:r.asset_id||'',catalogId:r.expense_catalog_id||'',accountId:r.account_id||'',assetCostTreatment:r.asset_cost_treatment==='capitalized'?'capitalized':'operating',date:r.expense_date||'',amount:num(r.amount),amountInclusive:num(r.amount_inclusive??r.amount),taxCode:String(r.tax_code||'none').toLowerCase()==='vat15'?'vat15':'none',taxAmount:num(r.tax_amount),paymentAccountId:r.payment_account_id||'',paymentMethodId:r.payment_method_id||'',notes:r.notes||'',movementGroupId:r.movement_group_id||'',movementSerial:r.movement_serial||'',systemGenerated:!!r.is_system_generated,systemKey:r.system_key||'',commissionRuleId:r.commission_rule_id||'',commissionNameAr:r.commission_name_ar_snapshot||'',commissionNameEn:r.commission_name_en_snapshot||'',commissionBeneficiaryType:r.commission_beneficiary_type_snapshot||'',commissionBeneficiaryName:r.commission_beneficiary_name_snapshot||'',commissionCalculationType:r.commission_calculation_type_snapshot||'',commissionCalculationValue:num(r.commission_calculation_value_snapshot),commissionTripValue:num(r.commission_trip_value_snapshot),createdAt:r.created_at||'',updatedAt:r.updated_at||''});
  const mapChartAccount=r=>({id:r.id,code:r.account_code||'',nameAr:r.name_ar||'',nameEn:r.name_en||'',parentId:r.parent_id||'',parentCode:r.parent_code||'',parentNameAr:r.parent_name_ar||'',parentNameEn:r.parent_name_en||'',rootClass:num(r.root_class),allowPosting:r.allow_posting===true,isSystem:r.is_system===true,isActive:r.is_active!==false,childCount:num(r.child_count),expenseCatalogCount:num(r.expense_catalog_count),transactionCount:num(r.transaction_count),sortOrder:num(r.sort_order),createdAt:r.created_at||'',updatedAt:r.updated_at||''});
  ...
  function recalcOptimisticAsset(snapshot,assetId){ const asset=snapshot.assets.find(x=>x.id===assetId); if(!asset)return; const rows=snapshot.expenses.filter(x=>x.assetId===assetId&&x.scope==='asset'),capitalized=rows.filter(x=>(x.assetCostTreatment||'capitalized')==='capitalized').reduce((a,x)=>a+num(x.amount),0),operating=rows.filter(x=>(x.assetCostTreatment||'capitalized')==='operating').reduce((a,x)=>a+num(x.amount),0); asset.capitalizedExpenses=capitalized; asset.operatingExpenses=operating; asset.currentValue=num(asset.initialValue)+capitalized; }
  ...
  async function fetchSection(name,c=client()){
    ...
    else if(name==='chartAccounts') out.chartAccounts=(await unwrap(c.from('sea_vibe_chart_accounts_view').select('*').order('account_code'),'SEA_VIBE_CHART_ACCOUNTS_LOAD_FAILED')).map(mapChartAccount);
  ...
  function expenseBatchArgs(records){
    ...
    return {scope,tripId,assetId,lines:rows.map(r=>({id:r.id||null,catalog_id:r.catalogId,date:r.date,amount:num(r.amount),tax_code:r.taxCode==='vat15'?'vat15':'none',payment_account_id:r.paymentAccountId||null,payment_method_id:r.paymentMethodId||null,notes:String(r.notes||'').trim()||null}))};
  }
  ...
  async function addExpenses(records,context={}){
    ...
    const applyLocal=async()=>{const groupId=localId('expense-group'),serial=`SV-MOV-OFF-${String(Date.now()).slice(-8)}`,filePayloads=[];for(const r of rows)filePayloads.push(await fileToPayload(r.file));optimistic(s=>{const treasury=s.chartAccounts.find(x=>String(x.code)==='1101'&&x.isActive&&x.allowPosting);rows.forEach(record=>{const catalog=s.expenseCatalog.find(x=>String(x.id)===String(record.catalogId)),assetCostTreatment=record.scope==='asset'?(catalog?.assetTreatment==='capitalized'?'capitalized':'operating'):'operating',inclusive=Number(num(record.amount).toFixed(2)),net=record.taxCode==='vat15'?Number((inclusive/1.15).toFixed(2)):inclusive,tax=record.taxCode==='vat15'?Number((inclusive-net).toFixed(2)):0,paymentAccountId=record.paymentAccountId||treasury?.id||'';s.expenses.unshift({id:localId('expense'),...record,amount:net,amountInclusive:inclusive,taxCode:record.taxCode==='vat15'?'vat15':'none',taxAmount:tax,paymentAccountId,accountId:record.accountId||catalog?.accountId||'',assetCostTreatment,movementGroupId:groupId,movementSerial:serial,systemGenerated:false,createdAt:nowIso(),updatedAt:nowIso()});});if(batch.scope==='trip')recalcOptimisticTrip(s,batch.tripId);if(batch.scope==='asset')recalcOptimisticAsset(s,batch.assetId);});return{groupId,serial,filePayloads};};
    const queueIt=async local=>enqueue('create',{kind:'expense_batch',mutation:'create',records:rows.map((r,i)=>({...r,file:null,filePayload:local.filePayloads[i]})),groupId:local.groupId,serial:local.serial,operationKey},local.groupId,await localDependencies(batch.tripId,batch.assetId,...rows.flatMap(r=>[r.catalogId,r.paymentMethodId,r.paymentAccountId]).filter(Boolean)),'',operationKey);
  ...
  async function setExpenseCatalogAccount(catalogId,accountId,{skipRefresh=false}={}){
    ...
    if(!skipRefresh)await refreshSections(['expenseCatalog','chartAccounts']);else markSyncSections(['expenseCatalog','chartAccounts']);
    return true;
  }
  async function saveReferenceOnline(kind,record,options={}){
    ...
    if(kind==='expenseCatalog'&&record.accountId)await setExpenseCatalogAccount(result.id,record.accountId,{skipRefresh:true});
    if(!options.skipRefresh)await refreshSections(kind==='expenseCatalog'?[section,'chartAccounts']:[section]);else markSyncSections(kind==='expenseCatalog'?[section,'chartAccounts']:[section]);
    return result.id;
  }
  ...
  async function saveChartAccount(record){
    const isUpdate=!!record.id;permission('seaVibeReference',isUpdate?'edit':'add');
    if(navigator.onLine===false)throw new Error('SEA_VIBE_ACCOUNT_ONLINE_REQUIRED');
    const id=await unwrap(client().rpc('sea_vibe_save_chart_account_r44r11',{p_id:record.id||null,p_account_code:String(record.code||'').trim(),p_name_ar:String(record.nameAr||'').trim(),p_name_en:String(record.nameEn||'').trim(),p_parent_id:record.parentId||null,p_allow_posting:record.allowPosting!==false,p_is_active:record.isActive!==false}),'SEA_VIBE_ACCOUNT_SAVE_FAILED');
    await audit(isUpdate?'update':'insert','sea_vibe_chart_accounts',id,{code:record.code,nameAr:record.nameAr,parentId:record.parentId||'',allowPosting:record.allowPosting!==false,isActive:record.isActive!==false});
    await refreshSections(['chartAccounts']);return id;
  }
  ...
  async function loadAccountStatementAccounts(){
    permission('seaVibeAccountStatement','view');
    if(navigator.onLine===false)throw new Error('SEA_VIBE_ACCOUNT_STATEMENT_ONLINE_REQUIRED');
    const rows=await unwrap(client().rpc('sea_vibe_account_statement_accounts_r44r38r15'),'SEA_VIBE_ACCOUNT_STATEMENT_ACCOUNTS_LOAD_FAILED');
    return (rows||[]).map(mapChartAccount);
  }
  async function loadAccountStatement(accountId,fromDate,toDate){
    permission('seaVibeAccountStatement','view');
    if(navigator.onLine===false)throw new Error('SEA_VIBE_ACCOUNT_STATEMENT_ONLINE_REQUIRED');
    if(!accountId)throw new Error('SEA_VIBE_ACCOUNT_STATEMENT_ACCOUNT_REQUIRED');
    if(!fromDate||!toDate)throw new Error('SEA_VIBE_ACCOUNT_STATEMENT_DATE_REQUIRED');
  ...
  async function handleQueuedMutation(operation,helpers){
    ...
    if(operation.action==='create'||operation.action==='update'){
      ...
      if(kind==='reference'){
        const r={...(p.record||{})},localRef=r.id||operation.localEntityId;let id=operation.action==='update'?await resolveQueuedId(localRef,operation,helpers):null;let base=operation.baseUpdatedAt||'';if(operation.action==='update'&&String(localRef||'').startsWith('local:')){const section=p.refKind==='paymentMethods'?'sea_vibe_payment_methods':'sea_vibe_expense_catalog';const row=await unwrap(client().from(section).select('updated_at').eq('id',id).single(),'تعذر التحقق من نسخة البيانات المرجعية');base=row?.updated_at||'';}const result=await syncMutationOnline('reference',mutation,operationKey,id,{refKind:p.refKind,nameAr:String(r.nameAr||'').trim(),nameEn:String(r.nameEn||'').trim(),isActive:r.isActive!==false},base);if(p.refKind==='expenseCatalog'&&r.accountId)await setExpenseCatalogAccount(result.id,r.accountId,{skipRefresh:true});markSyncSections(p.refKind==='expenseCatalog'?['expenseCatalog','chartAccounts']:[p.refKind==='paymentMethods'?'paymentMethods':'expenseCatalog']);return{id:result.id};
      }
      if(kind==='expense_batch'){
        const records=[];for(const raw of p.records||[]){const r={...raw};if(r.tripId)r.tripId=await resolveQueuedId(r.tripId,operation,helpers);if(r.assetId)r.assetId=await resolveQueuedId(r.assetId,operation,helpers);if(r.catalogId)r.catalogId=await resolveQueuedId(r.catalogId,operation,helpers);if(r.paymentMethodId)r.paymentMethodId=await resolveQueuedId(r.paymentMethodId,operation,helpers);if(r.paymentAccountId)r.paymentAccountId=await resolveQueuedId(r.paymentAccountId,operation,helpers);r.file=r.filePayload?payloadToFile(r.filePayload):null;records.push(r);}const result=await addExpensesOnline(records,{operationKey,skipRefresh:true});return{id:result?.movement_group_id||result?.id||operation.localEntityId};
      }
    ...
    if(operation.action==='expense_create'||operation.action==='expense_batch_create'){const rawRecords=operation.action==='expense_create'?[p]:p.records||[],records=[];for(const raw of rawRecords){const r={...raw};if(r.tripId)r.tripId=await resolveQueuedId(r.tripId,operation,helpers);if(r.assetId)r.assetId=await resolveQueuedId(r.assetId,operation,helpers);if(r.catalogId)r.catalogId=await resolveQueuedId(r.catalogId,operation,helpers);if(r.paymentMethodId)r.paymentMethodId=await resolveQueuedId(r.paymentMethodId,operation,helpers);if(r.paymentAccountId)r.paymentAccountId=await resolveQueuedId(r.paymentAccountId,operation,helpers);r.file=r.filePayload?payloadToFile(r.filePayload):null;records.push(r);}const result=await addExpensesOnline(records,{operationKey:legacyKey,skipRefresh:true});return{id:result?.movement_group_id||result?.expense_ids?.[0]||operation.localEntityId};}
    if(operation.action==='reference_create'||operation.action==='reference_update'){const r={...p.record},localRef=r.id||operation.localEntityId,id=operation.action==='reference_update'?await resolveQueuedId(localRef,operation,helpers):null,base=operation.action==='reference_update'?(operation.baseUpdatedAt||r.updatedAt||''):'';const result=await syncMutationOnline('reference',operation.action==='reference_update'?'update':'create',legacyKey,id,{refKind:p.kind,nameAr:String(r.nameAr||'').trim(),nameEn:String(r.nameEn||'').trim(),isActive:r.isActive!==false},base);if(p.kind==='expenseCatalog'&&r.accountId)await setExpenseCatalogAccount(result.id,r.accountId,{skipRefresh:true});markSyncSections(p.kind==='expenseCatalog'?['expenseCatalog','chartAccounts']:[p.kind==='tripTypes'?'tripTypes':p.kind==='paymentMethods'?'paymentMethods':'expenseCatalog']);return{id:result.id};}
