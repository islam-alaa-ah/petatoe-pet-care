(function(){
  ...
  const ALL_SECTIONS=['trips','expenses','assets','tripTypes','paymentMethods','expenseCatalog','permitFees','attachments','zawelTransactions','zawelBalance','treasuryMovements'];
  ...
  async function saveTripOnline(record,options={}){
    ...
    if(!options.skipRefresh)await refreshSections(['trips','expenses','zawelTransactions','zawelBalance','treasuryMovements']);
    else markSyncSections(['trips','expenses','zawelTransactions','zawelBalance','treasuryMovements']);
    return result.id;
  }
  async function saveTrip(record,context={}){
    ...
    const applyLocal=()=>{const id=record.id||localId('trip');optimistic(s=>{const old=s.trips.find(x=>x.id===id);const permit=calcPermitFee(record.peopleCount,record.durationHours,s);const other=s.expenses.filter(x=>x.tripId===id&&x.systemKey!=='sailing_permit').reduce((a,x)=>a+num(x.amount),0);const row={...old,...record,id,serial:record.serial||old?.serial||`SV-OFFLINE-${String(Date.now()).slice(-6)}`,status:record.status||old?.status||'open',tripExpenses:other+permit,netProfit:num(record.totalValue)-(other+permit),createdAt:old?.createdAt||nowIso(),updatedAt:nowIso()};syncOptimisticPermitWallet(s,id,row,old);s.trips=sortTrips([row,...s.trips.filter(x=>x.id!==id)]);const localPermit=s.expenses.find(x=>x.tripId===id&&x.systemKey==='sailing_permit');if(localPermit){localPermit.amount=permit;localPermit.date=record.date;localPermit.updatedAt=nowIso();}else{s.expenses.unshift({id:localId('permit'),scope:'trip',tripId:id,assetId:'',catalogId:s.expenseCatalog.find(x=>x.systemKey==='sailing_permit')?.id||'',date:record.date,amount:permit,paymentMethodId:'',notes:'',systemGenerated:true,systemKey:'sailing_permit',createdAt:nowIso(),updatedAt:nowIso()});}});return id;};
  ...
  async function saveReferenceOnline(kind,record,options={}){
    ...
    const baseUpdatedAt=options.baseUpdatedAt??record.baseUpdatedAt??existing?.updatedAt??'';
    const result=await syncMutationOnline('reference',isUpdate?'update':'create',operationKey,isUpdate?record.id:null,{refKind:kind,nameAr:String(record.nameAr||'').trim(),nameEn:String(record.nameEn||'').trim(),isActive:record.isActive!==false},baseUpdatedAt);
  ...
  async function saveReference(kind,record,context={}){
    ...
    const applyLocal=()=>{const id=record.id||localId(`ref-${kind}`);optimistic(s=>{const rows=referenceRows(s,kind),old=rows.find(x=>x.id===id),next={...old,...record,id,isActive:record.isActive!==false,updatedAt:nowIso()},filtered=rows.filter(x=>x.id!==id);if(kind==='tripTypes')s.tripTypes=[next,...filtered];else if(kind==='paymentMethods')s.paymentMethods=[next,...filtered];else s.expenseCatalog=[next,...filtered];});return id;};
    const queueIt=async id=>enqueue(isUpdate?'update':'create',{kind:'reference',mutation:isUpdate?'update':'create',refKind:kind,record:{...record,id},operationKey},id,await localDependencies(record.id),baseUpdatedAt,operationKey);
    if(!context.skipOfflineQueue&&navigator.onLine===false){const id=applyLocal();await queueIt(id);return id;}
  ...
  async function handleQueuedMutation(operation,helpers){
    ...
    if(operation.action==='create'||operation.action==='update'){
      ...
      if(kind==='reference'){
        const r={...(p.record||{})},localRef=r.id||operation.localEntityId;let id=operation.action==='update'?await resolveQueuedId(localRef,operation,helpers):null;let base=operation.baseUpdatedAt||'';if(operation.action==='update'&&String(localRef||'').startsWith('local:')){const section=p.refKind==='tripTypes'?'sea_vibe_trip_types':p.refKind==='paymentMethods'?'sea_vibe_payment_methods':'sea_vibe_expense_catalog';const row=await unwrap(client().from(section).select('updated_at').eq('id',id).single(),'تعذر التحقق من نسخة البيانات المرجعية');base=row?.updated_at||'';}const result=await syncMutationOnline('reference',mutation,operationKey,id,{refKind:p.refKind,nameAr:String(r.nameAr||'').trim(),nameEn:String(r.nameEn||'').trim(),isActive:r.isActive!==false},base);markSyncSections([p.refKind==='tripTypes'?'tripTypes':p.refKind==='paymentMethods'?'paymentMethods':'expenseCatalog']);return{id:result.id};
      }
  ...
  window.SeaVibeService=Object.freeze({load,refresh,getSnapshot,getReadStatus,invalidate,saveTrip,setTripStatus,saveAsset,addExpenses,getExpenseMovement,updateExpenseMovement,deleteExpenseMovement,deleteExpense,saveReference,savePermitFee,savePermitFees,topupZawel,updateZawelTopup,deleteZawelTopup,signedAttachment});
})();
