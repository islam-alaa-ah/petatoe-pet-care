(function(){
  ...
  async function addExpenses(records,context={}){
    ...
    const applyLocal=async()=>{const groupId=localId('expense-group'),serial=`SV-MOV-OFF-${String(Date.now()).slice(-8)}`,filePayloads=[];for(const r of rows)filePayloads.push(await fileToPayload(r.file));optimistic(s=>{rows.forEach(record=>{const catalog=s.expenseCatalog.find(x=>String(x.id)===String(record.catalogId)),assetCostTreatment=record.scope==='asset'?(catalog?.assetTreatment==='capitalized'?'capitalized':'operating'):'operating';s.expenses.unshift({id:localId('expense'),...record,accountId:record.accountId||catalog?.accountId||'',assetCostTreatment,movementGroupId:groupId,movementSerial:serial,systemGenerated:false,createdAt:nowIso(),updatedAt:nowIso()});});if(batch.scope==='trip')recalcOptimisticTrip(s,batch.tripId);if(batch.scope==='asset')recalcOptimisticAsset(s,batch.assetId);});return{groupId,serial,filePayloads};};
