...
(function () {
  ...
  const followupMethodLabel=value=>{const map={'اتصال':'followups.method.call','واتساب':'followups.method.whatsapp','زيارة':'followups.method.visit','بريد إلكتروني':'followups.method.email','اجتماع':'followups.method.meeting'};return map[value]?t(map[value],String(value||'')):String(value||'')};
  const followupResultLabel=value=>{const map={'تم التواصل':'followups.result.contacted','لم يتم الرد':'followups.result.noAnswer','طلب عقد':'followups.result.requestContract','تم إرسال عقد':'followups.result.sentContract','تفاوض':'followups.result.negotiation','تم البيع':'followups.result.sold','لم يتم البيع':'followups.result.notSold','مؤجل':'followups.result.deferred'};return map[value]?t(map[value],String(value||'')):String(value||'')};
  const quotationStatusLabel=value=>{const raw=String(value||'').trim();const canonical=['قيد التنفيذ','مقبول','مرفوض'].includes(raw)?raw:['تحت التجهيز','تم الإرسال','تحت المراجعة'].includes(raw)?'قيد التنفيذ':raw==='ملغي'?'مرفوض':raw;const map={'قيد التنفيذ':'contracts.status.inProgress','مقبول':'contracts.status.accepted','مرفوض':'contracts.status.rejected'};return map[canonical]?t(map[canonical],canonical):canonical};
  function safe(value) {
    return String(value ?? "")
      .replace(/[&<>"]/g, character => ({
        "&": "&amp;",
  ...
  function addressLabel(customer = {}) {
    return window.KYUMGeography?.localizedDistrictLabel?.(
      customer.neighborhoodId || customer.neighborhood_id || "",
      customer.address || ""
    ) || customer.address || "";
  }
  ...
  function summaryRows(view) {
    ...
    return [
      ...
      [t('customer360.export.code','Code'),customer.customerNumber||''],[t('customer360.export.name','Name'),customer.name||''],[t('customer360.export.address','Address'),addressLabel(customer)||''],[t('customer360.export.mobile','Mobile'),customer.phone||''],
  ...
  function createExcel(view) {
    ...
    const followupRows=[[t('customer360.export.date','Date'),t('customer360.export.method','Method'),t('customer360.export.result','Result'),t('customer360.export.representative','Representative'),t('customer360.export.nextFollowup','Next Follow-up'),t('customer360.export.completed','Completed'),t('customer360.export.notes','Notes')],...view.followups.map(item=>[item.contactDate||item.createdAt||'',item.method?followupMethodLabel(item.method):'',item.result?followupResultLabel(item.result):'',item.representative||'',item.nextFollowupDate||'',item.completed?t('customer360.export.yes','Yes'):t('customer360.export.no','No'),item.notes||''])];
    ...
    const quotationRows=[[t('customer360.export.quotation','Quotation'),t('customer360.export.date','Date'),t('customer360.export.status','Status'),t('customer360.export.amount','Amount'),t('customer360.export.representative','Representative'),t('customer360.export.rejectionReason','Rejection / No Sale Reason')],...view.quotations.map(item=>[item.code||item.quotationNumber||'',item.quotationDate||item.createdAt||'',item.status?quotationStatusLabel(item.status):'',Number(item.amount||0),item.representative||'',item.rejectionReason||item.noSaleReason||''])];
