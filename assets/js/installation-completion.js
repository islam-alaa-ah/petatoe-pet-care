(()=>{
  ...
  function collectionAttachments(r){const visitIds=new Set((r.groupVisitIds?.length?r.groupVisitIds:[r.visitId]).filter(Boolean).map(String));return (r.executionFiles||[]).filter(f=>String(f.fileKind||'')==='collection'&&(!f.visitId||!visitIds.size||visitIds.has(String(f.visitId))))}
  function openCompletionAttachments(r){const files=collectionAttachments(r),box=$("appointmentAttachmentsList");$("appointmentAttachmentsTitle").textContent=t('appointments.completion.attachments','مرفقات التحصيل');$("appointmentAttachmentsSubtitle").textContent=`${r.executionNumber||r.requestNumber} — ${r.customerName||''}`;box.innerHTML=files.length?files.map((f,i)=>`<div class="sales-invoice-attachment-item"><div><strong>${esc(t('appointments.completion.collectionAttachment','مرفق التحصيل'))}</strong><small>${esc(f.originalName||`مرفق ${i+1}`)}</small></div><button class="secondary-btn compact-btn" type="button" data-open-execution-file="${esc(f.storagePath)}">${esc(t('invoices.attachments.open','فتح'))}</button></div>`).join(''):`<p class="empty-state">${esc(t('invoices.attachments.none','لا توجد مرفقات'))}</p>`;$("appointmentAttachmentsDialog")?.showModal()}
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    $("installationCompletionTableBody")?.addEventListener("click",e=>{
      ...
      if(cancelBtn){
        ...
        window.InstallationsServiceSafe.cancelConfirmedQuantity({id:r.id,visitId:r.visitId,visitIds:r.groupVisitIds||[r.visitId],reason}).then(()=>load()).catch(err=>status($("installationCompletionStatus"),err.message,"error")).finally(()=>{cancelBtn.disabled=false});
        return;
      }
    ...
    $("installationQuantityInvoiceNumber")?.addEventListener("input",e=>{e.target.value=e.target.value.replace(/\D/g,"").slice(0,9)});
    ...
    $("installationQuantityConfirmationForm")?.addEventListener("submit",async e=>{
      ...
      try{
        ...
        if(directInvoiceNumber&&directInvoiceNumber.length!==9)throw new Error('رقم الفاتورة يجب أن يتكون من 9 أرقام، أو اختر بدون فاتورة.');
