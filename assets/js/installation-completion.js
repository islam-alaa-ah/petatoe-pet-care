(()=>{
  ...
  function serviceTypeOptions(selected=''){return `<option value="">${esc(t('appointmentNew.services.select','Select service'))}</option>`+quantityOptions.serviceTypes.map(x=>`<option value="${esc(x.id)}" ${String(x.id)===String(selected)?'selected':''}>${esc(x.name)}</option>`).join('')}
