(function(){
  ...
  async function executionWorkspace(){
    ...
    const neighborhoodResult=executionNeighborhoodIds.length?await db().from('installation_neighborhoods').select('id,name,name_en').in('id',executionNeighborhoodIds):{data:[],error:null};
    if(neighborhoodResult.error)throw new Error('تعذر تحميل ترجمة أحياء التنفيذ: '+neighborhoodResult.error.message);
    const executionNeighborhoods=new Map((neighborhoodResult.data||[]).map(n=>[String(n.id),{name:n.name||'',nameEn:n.name_en||''}]));
