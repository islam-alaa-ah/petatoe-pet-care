(function(){
  ...
  async function completionList(){
    ...
    const [requests,reports,files,executionFiles,invoices,serviceRows,pendingVisits,confirmedHistoryVisits,scheduledVisits,scheduledVisitLines,collectionRows]=await Promise.all([
      ...
      fetchPaged((from,to)=>db().from('installation_completion_reports').select('*').range(from,to)),fetchPaged((from,to)=>db().from('installation_completion_files').select('*').range(from,to),1000),fetchPaged((from,to)=>db().from('installation_execution_files').select('id,installation_request_id,execution_visit_id,storage_path,original_name,mime_type,file_size,file_kind,uploaded_at').range(from,to),1000),fetchPaged((from,to)=>db().rpc('get_installation_completion_invoice_markers').range(from,to),1000),fetchPaged((from,to)=>db().from('installation_request_services').select('id,installation_request_id,quantity,line_total,service:installation_service_types(default_cost)').range(from,to),1000),
