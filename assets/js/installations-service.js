(function(){
  ...
  async function saveCompletionWorkspace(payload){
    ...
    const {data,error}=await db().rpc('update_installation_completion_workspace',{p_request_id:payload.id,p_visit_id:payload.visitId||null,p_services:services,p_discount_amount:discount,p_collection:{amount_collected:amount,payment_method:String(payload.collection?.paymentMethod||'').trim(),collection_reference:String(payload.collection?.reference||'').trim(),collection_notes:String(payload.collection?.notes||'').trim()}});
  ...
  async function installationSummaryReport(filters={}){
    ...
    const requestSelect='id,request_number,neighborhood_id,scheduled_date,scheduled_time,installation_team_id,assigned_technician_name,representative_id,status,total_services_amount,on_route_at,map_opened_at,arrived_at,started_at,collection_at,completed_at,customer:customers(id,customer_name,phone),team:installation_teams(id,name),representative:sales_representatives(id,full_name)';
