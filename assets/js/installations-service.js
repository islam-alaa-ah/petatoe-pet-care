(function(){
  ...
  async function executionWorkspace(){
    ...
    const [requestResult,visitResult,anyVisitResult,lineResult,currentResult]=await Promise.all([
      ...
      db().from('installation_execution_visits').select('id,installation_request_id,visit_no,scheduled_date,scheduled_time,installation_team_id,technician_name,status,selected_for_execution_at,selected_for_execution_by,on_route_at,map_opened_at,arrived_at,started_at,completed_at,execution_notes,team:installation_teams(id,name,groomer_name,driver_name)').in('status',['مجدولة','قيد التنفيذ','بانتظار التأكيد']).order('scheduled_date',{ascending:true}).order('scheduled_time',{ascending:true}),
