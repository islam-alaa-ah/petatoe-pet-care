(function(){
  ...
  async function installationSummaryReport(filters={}){
    ...
    const addLine=(teamId,teamName,entryKey,serviceName,quantity,unitPrice,unitCost,financialFactor=1)=>{
    ...
    for(const request of singleDayRequests){
      for(const service of servicesByRequest.get(String(request.id||''))||[]){
        ...
        addLine(request.installation_team_id,request.team?.name,'request:'+request.id,service.service?.name,quantity,unitPrice,Number(service.service?.default_cost||0),requestFinancialFactor(request));
      }
    }
    ...
    const salesInvoices=(salesInvoiceRows||[]).map(invoice=>{
      ...
      const teamName=anchorVisit?.team?.name||request.team?.name||grouped.get(teamId)?.name||'غير مسند';
      ...
      return {id:invoice.id,requestId:invoice.installation_request_id||'',requestNumber:invoice.request_number||request.request_number||'',teamId,teamName,paymentMethod,invoiceNumber:invoice.is_without_invoice?'بدون فاتورة':(invoice.invoice_number||'—'),isWithoutInvoice:Boolean(invoice.is_without_invoice),invoiceDate:invoice.invoice_date||'',invoiceAmountBeforeTax:Number(invoice.invoice_amount||0),invoiceAmountInclTax:Math.round(Number(invoice.invoice_amount||0)*1.15),services};
