(function(){
  ...
  async function completionList(){
    ...
    const [requests,reports,files,executionFiles,invoices,serviceRows,pendingVisits,confirmedHistoryVisits,scheduledVisits,scheduledVisitLines,collectionRows]=await Promise.all([
      ...
      fetchPaged((from,to)=>db().from('installation_completion_reports').select('*').range(from,to)),fetchPaged((from,to)=>db().from('installation_completion_files').select('*').range(from,to),1000),fetchPaged((from,to)=>db().rpc('get_installation_completion_execution_files').range(from,to),1000),fetchPaged((from,to)=>db().rpc('get_installation_completion_invoice_markers').range(from,to),1000),fetchPaged((from,to)=>db().from('installation_request_services').select('id,installation_request_id,quantity,line_total,service:installation_service_types(default_cost)').range(from,to),1000),
    ...
    return [...groupedOut.values()].map(row=>{if(!row.visitId)return row;row.groupVisitIds=[...new Set(row.groupVisitIds||[])];row.confirmedVisitIds=[...new Set(row.confirmedVisitIds||[])];row.quantityConfirmed=Number(row._confirmedCount||0)===Number(row._memberCount||1);row.confirmedHistory=row.quantityConfirmed&&row.confirmedVisitIds.length>0;if(row.groupVisitIds.length>1)row.executionNumber=row.requestNumber;delete row._memberCount;delete row._confirmedCount;return row;});
  }
