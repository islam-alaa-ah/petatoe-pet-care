...
const AR=/[\u0600-\u06FF]/;
const ALLOWED_PREFIXES=new Set(['common','shared','sidebar','dashboard','customers','followups','contracts','invoices','users','salaryStatement','commissionStatement','commission','payroll','appointments','appointmentNew','execution','vehicleTreasury','seaVibePayroll','seaVibe','translationCenter']);
...
for(const required of ["const routeFor=(key)=>","value.startsWith('appointmentNew.')","value.startsWith('vehicleTreasury.')","value.startsWith('seaVibePayroll.')","value.startsWith('translationCenter.')","moduleNameFor=key=>routeFor(key).moduleName"]){if(!source.includes(required))fail(`routing contract missing: ${required}`);}
