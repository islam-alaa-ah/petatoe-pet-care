...
for(const required of ["const routeFor=(key)=>","value.startsWith('appointmentNew.')","value.startsWith('vehicleTreasury.')","value.startsWith('seaVibePayroll.')","value.startsWith('translationCenter.')","value.startsWith('representatives.')","value.startsWith('referenceData.')","moduleNameFor=key=>routeFor(key).moduleName"]){if(!source.includes(required))fail(`routing contract missing: ${required}`);}
